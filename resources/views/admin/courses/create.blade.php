    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.courses.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('type_course') ? 'is-invalid' : '' }}" type="text" name="type_course" id="type_course" value="{{ old('type_course', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('trainee') ? 'is-invalid' : '' }}" name="trainee_id" id="trainee_id">
                    @foreach($trainees as $id => $trainee)
                        <option value="{{ $id }}" {{ old('trainee_id') == $id ? 'selected' : '' }}>{{ $trainee }}</option>
                    @endforeach
                </select>
